...
| `task` | `troubleshooting` — common errors after the procedure | A "Set up the bundle" task page ending with "Common issues" entries. |
| `concept` | `task` — brief how-to showing the concept in action | An "Adapters" concept page including a short "Render a controller fragment" procedure. |
| `concept` | `reference` — summary table of related API surface | A "Configuration" concept page ending with a table of bundle options. |
